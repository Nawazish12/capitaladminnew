...
export default function NeighbourhoodDialog({
  open,
  handleOpen,
  handleClose,
  type,
  ...
  const { mutate: addNeighbourhood, isLoading: addLoading } = useMutation(
    (body) => axios.post("/neighbourhood", body),
    {
      onSuccess: (res) => {
        if (res?.data?.success) {
  ...
  const { mutate: editNeighbourhood, isLoading: editLoading } = useMutation(
    (body) => axios.post(`/neighbourhood/${dataForEdit.id}`, body),
    {
      onSuccess: (res) => {
        if (res?.data?.success) {
          enqueueSnackbar(res?.data?.message, {
  ...
  const onSubmit = (model) => {
    ...
    if (type === "add") {
      addNeighbourhood(body);
    } else {
      editNeighbourhood(editPayload);
    }
  };
