...
export default function CommunityLookUpDialog({
  open,
  handleOpen,
  handleClose,
  type,
  ...
  const { mutate: addCycleTrack, isLoading: addLoading } = useMutation(
    (body) => axios.post("/community_facility", body),
    {
      onSuccess: (res) => {
        if (res?.data?.success) {
  ...
  const { mutate: editCycleTrack, isLoading: editLoading } = useMutation(
    (body) => axios.post(`/community_facility/${dataForEdit.id}`, body),
    {
      onSuccess: (res) => {
        if (res?.data?.success) {
          enqueueSnackbar(res?.data?.message, {
